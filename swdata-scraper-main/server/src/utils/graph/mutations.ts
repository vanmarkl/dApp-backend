...
const UPDATE_COINGECKO_CHECK_DAILIES = gql`
  mutation UpdateCoinGeckoCheck(
    $ids: [Int!] = []
    $calledCoinGeckoDailies: Boolean = false
  ) {
    update_prices_tokens(
      where: { id: { _in: $ids } }
      _set: { calledCoinGeckoDailies: $calledCoinGeckoDailies }
    ) {
      affected_rows
    }
  }
...
const UPDATE_COINGECKO_CHECK_HOURLIES = gql`
  mutation UpdateCoinGeckoCheck(
    $ids: [Int!] = []
    $calledCoinGeckoHourlies: Boolean = false
  ) {
    update_prices_tokens(
      where: { id: { _in: $ids } }
      _set: { calledCoinGeckoHourlies: $calledCoinGeckoHourlies }
    ) {
      affected_rows
    }
  }
...
const UPDATE_COINGECKO_CHECK_MINUTES = gql`
  mutation UpdateCoinGeckoCheck(
    $ids: [Int!] = []
    $calledCoinGeckoMinutes: Boolean = false
  ) {
    update_prices_tokens(
      where: { id: { _in: $ids } }
      _set: { calledCoinGeckoMinutes: $calledCoinGeckoMinutes }
    ) {
      affected_rows
    }
  }
...
export {
  ...
  UPDATE_COINGECKO_CHECK_DAILIES,
  UPDATE_COINGECKO_CHECK_HOURLIES,
  UPDATE_COINGECKO_CHECK_MINUTES,
};
