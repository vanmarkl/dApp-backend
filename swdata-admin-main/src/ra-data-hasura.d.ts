declare module "ra-data-hasura" {
  ...
  type FetchActionsPaginated =
    | FetchType.GET_MANY_REFERENCE
    | FetchType.GET_LIST;
  type FetchMutateSingle =
    | FetchType.CREATE
    | FetchType.UPDATE
    | FetchType.DELETE;
  ...
  type ResponseParser =
    | ((
        ...
        f: FetchActionsPaginated
      ) => ({ data }: any) => { data: object[]; total: number })
    | ((
        i: IntrospectionResult
      ) => (f: FetchType.GET_MANY) => ({ data }: any) => { data: object[] })
    | ((
        i: IntrospectionResult
      ) => (f: FetchMutateSingle) => ({ data }: any) => { data: object })
    | ((
        i: IntrospectionResult
      ) => (f: FetchMutateBatch) => ({ data }: any) => { data: string[] });
  ...
  export function buildQueryFactory(
    buildVariables: BuildVariables,
    buildGqlQuery: BuildGqlQuery,
